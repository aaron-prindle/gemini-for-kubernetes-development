...
function App() {
  ...
  // NEW: Fetch user identity on load
  useEffect(() => {
    fetch('/api/whoami')
      .then(res => {
      ...
      .catch(err => console.error("Failed to fetch user identity:", err));
  }, []);
    ...
    <div className="App">
      <header className="App-header">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          {/* NEW: Display logged-in user */}
          {user && (
            <div className="user-info">
              Logged in as: <strong>{user.user}</strong>
            </div>
          )}
        </div>
      </header>
