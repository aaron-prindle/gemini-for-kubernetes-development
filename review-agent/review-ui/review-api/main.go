...
// AuthMiddleware extracts user identity from headers injected by oauth2-proxy
func AuthMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		user := c.GetHeader("X-Forwarded-User")
		email := c.GetHeader("X-Forwarded-Email")

		// Fallback for local development when not running behind oauth2-proxy
		if user == "" && os.Getenv("GIN_MODE") != "release" {
			// Check for an explicit local dev user override, or default to anonymous-dev
			if devUser := os.Getenv("DEV_USER"); devUser != "" {
				user = devUser
			} else {
				user = "anonymous-dev"
			}
			log.Printf("WARNING: No auth headers found in dev mode, using fallback user: %s", user)
		}

		// In production, strictly enforce the presence of the user header
		if user == "" {
			log.Println("Error: No X-Forwarded-User header found. Is oauth2-proxy configured correctly?")
			c.AbortWithStatusJSON(http.StatusUnauthorized, gin.H{"error": "unauthorized - missing identity headers"})
			return
		}

		// Store identity in context for subsequent handlers
		c.Set("user", user)
		c.Set("email", email)
		c.Next()
	}
...
func main() {
	...
	// Add middleware to log requests, authenticate, and log responses
...
func getIssues(c *gin.Context) {
	...
	for iter.Next(c.Request.Context()) {
		key := iter.Val()
		// key is issue:repo:REPO:issue:ISSUEID:handler:HANDLER
...
func saveIssueDraft(c *gin.Context) {
	...
	issueKey := fmt.Sprintf("issue:repo:%s:issue:%s:handler:%s", repo, issueID, handler)
